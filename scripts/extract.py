...
class FSBExtractor:
    ...
    def write_to_file(self, filename_prefix, filename, extension, contents):
        ...
        if filename_prefix:
            path = os.path.join(
                directory, "{0}-{1}.{2}".format(filename_prefix, filename, extension)
            )
        else:
            path = os.path.join(directory, "{0}.{1}".format(filename, extension))
        path = self.find_valid_path(
            path
        )  # rename to avoid overwriting files with same filename
    ...
    def load_fsb(self, data):
        ...
        self.debug(
            "\tNamed samples: %s" % ("Yes" if fsb.header.nameTableSize else "No")
        )
    ...
    def read_samples(self, fsb_name, fsb, ext):
        ...
        for sample in fsb.samples:
            ...
            try:
                yield sample_fakepath, "{0}_{1}-{2}ch-{3}smpl".format(
                    sample.name, sample.frequency, sample.channels, sample.samples
    ...
    def handle_file(self, f):
        ...
        while data:
            ...
            for sample_fakepath, sample_name, sample_data in self.read_samples(
                fakepath_prefix, fsb, ext
            ):
                outpath = self.write_to_file(
                    sample_prefix, sample_name, ext, sample_data
                )
