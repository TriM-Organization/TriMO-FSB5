...
def main():
    pp = argparse.ArgumentParser(
        description="Dump FSB5 vorbis headers from a 32-bit Windows executable"
    )
    ...
    for section in pe.sections:
        ...
        if pe.adjust_FileAlignment(
            section.PointerToRawData, pe.OPTIONAL_HEADER.FileAlignment
        ) > len(pe.__data__):

            continue

        VirtualAddress_adj = pe.adjust_SectionAlignment(
    ...
    table_addr = 0
    for ref in refs:
        if (ref - 24 not in refs and ref - 12 not in refs) and (
            ref + 24 in refs or ref + 36 in refs
        ):
            table_addr = ref
            break
    ...
    while True:
        ea_ofs = ea - base_addr
        buf1, len1, crc, buf2, ofs, len2 = struct.unpack(
            "<IIIIII", mapped_data[ea_ofs : ea_ofs + 0x18]
        )
